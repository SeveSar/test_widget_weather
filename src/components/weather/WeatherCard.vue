<template>
  <div class="weather-card">
    <h3 class="weather-card__city">{{ weatherItem.name }}, {{ weatherItem.sys.country }}</h3>
    <div class="weather-card__header">
      <img
        :src="`https://openweathermap.org/img/wn/${weatherItem.weather[0].icon}@2x.png`"
        alt=""
        class="weather-card__img"
      />
      <h2 class="weather-card__grade">{{ Math.round(weatherItem.main?.temp) }} °C</h2>
    </div>
    <p class="weather-card__descr">
      Feels like {{ Math.round(weatherItem.main?.feels_like) }}°C.
      <span> {{ weatherItem.weather[0].description }}.</span>
    </p>
    <div class="weather-card__extra weather-card__row">
      <div class="weather-card__row-item">
        <svg
          data-v-47880d39=""
        ...
        {{ Math.round(weatherItem.wind.speed) }} m/s
      </div>
      <div class="weather-card__row-item">
        <svg
          data-v-7bdd0738=""
    ...
    <div class="weather-card__extra weather-card__row weather-card__row--wrap">
      <div class="weather-card__row-item">
        Humidity: {{ Math.round(weatherItem.main?.humidity) }}%
      </div>
      <div class="weather-card__row-item">Visibility: {{ weatherItem.visibility / 1000 }} km</div>
    </div>
    <slot></slot>
  </div>
</template>
